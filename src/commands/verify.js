...
const secrets = require('../utils/config.js').loadsecrets();
const config = require('../utils/config.js').loadconfig();
...
module.exports = {
    name: 'verify',
    description:
        'Verifies that the executed user is the Minecraft user provided and gives him the corresponding roles',
        ...
        try {
            ...
            if (!player.player.socialMedia)
                return await message.channel.send(
                    `You havent set your Hypixel Discord Name yet! Please set it to ${message.author.tag}!`
                );
            if (!player.player.socialMedia.links)
                return await message.channel.send(
                    `You havent set your Hypixel Discord Name yet! Please set it to ${message.author.tag}!`
                );
            ...
            if (player.player.socialMedia.links.DISCORD !== message.author.tag)
                return await message.channel.send(
                    `You haven't set your Hypixel Discord Name to ${message.author.tag}!`
                );
            ...
            if (player.player.rank) {
                ...
                player.player.monthlyPackageRank &&
            ...
            if (roles.cache.some((rle) => rle.id == role_id)) {
                ...
                return await message.channel.send(
                    `You're already verified and there have been no rank changes`
                );
            }
            if (!roles.cache.some((rle) => rle.id == role_id)) {
            ...
            await message.react('❌');
            return await message.channel.send(
                `:x: Error 404: Player '${args[0]}' not found!`
            );
        }
    },
};
