...
module.exports = {
    ...
    async execute(message, args) {
        ...
        try {
            ...
            var item = Object.values(Bazaar.products).find(x => x.product_id == args[0].toUpperCase());
            var itemembed = new MessageEmbed()
                .setColor('#a2eeef')
                .setTitle(`Bazaar Info for ${args[0].toUpperCase()}`)
                .addFields(
                    { name: 'Sell Info', value: `**Sell Price**\n${Number(item.quick_status.sellPrice).toFixed(1)}\n\n**Sell Volume**\n${item.quick_status.sellVolume}\n\n**Sell Offers**\n${item.quick_status.sellOrders}`, inline: true},
                    { name: 'Buy Info', value: `**Buy Price** \n${Number(item.quick_status.buyPrice).toFixed(1)} \n\n**Buy Volume** \n${item.quick_status.buyVolume} \n\n**Buy Orders** \n${item.quick_status.buyOrders}`, inline: true}
                )
